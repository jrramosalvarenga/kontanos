-- Fix demo provider profiles (users already exist)
-- Insert provider profiles using subqueries to get user IDs
...
-- Also insert the demo main user profile
INSERT INTO provider_profiles (user_id, slug, full_name, tagline, bio, avatar_url, phone, whatsapp, category_id, location_id, years_experience, is_featured, is_verified, rating_avg, rating_count, profile_views, response_time)
SELECT u.id, 'demo-kontactanos', 'Demo Kontactanos', 'Perfil de demostración de la plataforma',
  'Este es el perfil demo de Kontactanos para mostrar las funcionalidades de la plataforma.',
  'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=300&h=300&fit=crop&crop=face',
  '+504 9999 0000', '+504 9999 0000',
  (SELECT id FROM categories WHERE slug='tecnologia-it'),
  (SELECT id FROM locations WHERE slug='honduras-fm-tegucigalpa'),
  3, FALSE, TRUE, 4.5, 12, 200, '< 3 horas'
FROM users u WHERE u.email='demo@kontactanos.com'
ON CONFLICT (slug) DO NOTHING;
...
INSERT INTO reviews (provider_id, reviewer_name, rating, comment)
SELECT pp.id, 'Valentina Cruz', 5, 'Sofía es increíble! Mi cabello nunca se había visto tan bien. Definitivamente volveré.'
FROM provider_profiles pp WHERE pp.slug = 'sofia-peluqueria-estilista';

INSERT INTO reviews (provider_id, reviewer_name, rating, comment)
SELECT pp.id, 'Ricardo Morales', 4, 'Jorge es muy profesional. Resolvió todos mis temas fiscales sin complicaciones.'
FROM provider_profiles pp WHERE pp.slug = 'jorge-contador-finanzas';

INSERT INTO reviews (provider_id, reviewer_name, rating, comment)
SELECT pp.id, 'Daniela Pérez', 5, 'Laura es una profesora excepcional. Aprobé el IELTS con 7.5 gracias a ella!'
FROM provider_profiles pp WHERE pp.slug = 'laura-profesora-ingles';
