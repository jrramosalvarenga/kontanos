...
$products  = getProviderProducts($provider['id']);
$services  = getProviderServices($provider['id']);
$catalog   = array_merge(
    array_map(fn($p) => $p + ['_kind' => 'product'], $products),
    array_map(fn($s) => $s + ['_kind' => 'service'], $services)
);
...
<section class="profile-hero text-white">
    ...
    <!-- Share row in hero -->
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-6 flex items-center justify-end gap-2">
        <span class="text-xs text-brand-300 hidden sm:inline">¿Lo conoces? Compártelo:</span>
        <a href="https://wa.me/?text=<?= urlencode('Te recomiendo a ' . $provider['full_name'] . ($provider['tagline'] ? ' — ' . $provider['tagline'] : '') . '. Mira su perfil en Kontactanos: ' . $profileUrl) ?>"
           target="_blank" rel="noopener"
           class="flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1ebe5c] text-white text-xs font-semibold px-3 py-1.5 rounded-full transition-colors">
        ...
        <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($profileUrl) ?>"
           target="_blank" rel="noopener"
           class="flex items-center gap-1.5 bg-[#1877F2] hover:bg-[#1464d0] text-white text-xs font-semibold px-3 py-1.5 rounded-full transition-colors">
            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            Facebook
        </a>
        <button onclick="nativeShare()"
                class="flex items-center gap-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-semibold px-3 py-1.5 rounded-full transition-colors border border-white/30">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    ...
                    <div class="grid sm:grid-cols-2 gap-3">
                        <?php foreach ($catalog as $item):
                            ...
                            $waText  = "Hola, me interesa: {$item['nombre']} de tu perfil en Kontanos";
                            $waUrl   = $provider['whatsapp']
                                ? 'https://wa.me/' . preg_replace('/\D/', '', $provider['whatsapp']) . '?text=' . urlencode($waText)
                                : null;
                        ?>
                        <div class="rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                            <?php if ($item['imagen']): ?>
                            <img src="<?= e($item['imagen']) ?>" alt="<?= e($item['nombre']) ?>" class="w-full h-36 object-cover" loading="lazy">
                            <?php endif; ?>
                            <div class="p-4">
                                <div class="flex items-start justify-between gap-2 mb-1">
                                    <h3 class="font-semibold text-gray-900 text-sm leading-snug"><?= e($item['nombre']) ?></h3>
                                    <?php if ($price): ?>
                                    <span class="font-bold text-brand-700 text-sm flex-shrink-0"><?= e($price) ?></span>
                                    <?php endif; ?>
                                </div>
                                <?php if ($item['descripcion']): ?>
                                <p class="text-xs text-gray-500 line-clamp-2 mb-3"><?= e($item['descripcion']) ?></p>
                                <?php endif; ?>
                                <?php if ($waUrl): ?>
                                <a href="<?= e($waUrl) ?>" target="_blank" rel="noopener"
                                   class="tap-btn w-full gap-1.5 bg-green-50 text-green-700 text-xs font-bold py-2.5 rounded-xl hover:bg-green-100 transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                    Pedir por WhatsApp
                                </a>
                                <?php endif; ?>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                    <h3 class="font-semibold text-gray-900 mb-1 text-sm">Recomienda este perfil</h3>
                    <p class="text-xs text-gray-500 mb-3">Ayuda a <?= e($provider['full_name']) ?> a llegar a más clientes</p>
                    <div class="grid grid-cols-2 gap-2">
                        <a href="https://wa.me/?text=<?= urlencode('Te recomiendo a ' . $provider['full_name'] . ($provider['tagline'] ? ' — ' . $provider['tagline'] : '') . '. Puedes contactarlo directamente aquí: ' . $profileUrl) ?>"
...
function nativeShare() {
    const data = {
        title: <?= json_encode($provider['full_name'] . ' en Kontactanos') ?>,
        text: <?= json_encode(($provider['tagline'] ?: 'Mira este profesional en Kontactanos') . ' — Contáctalo directamente:') ?>,
        url: <?= json_encode($profileUrl) ?>
    };
    if (navigator.share) {
        navigator.share(data).catch(() => {});
    } else {
        navigator.clipboard.writeText(data.url).then(() => {
            showToast('¡Enlace copiado! Compártelo donde quieras.');
        });
    }
}
</script>
...
<div class="fixed bottom-0 left-0 right-0 z-40 lg:hidden pointer-events-none" id="float-share">
    <div class="bg-white border-t border-gray-200 shadow-xl px-4 py-3 flex items-center gap-3 pointer-events-auto">
        <div class="flex-1 min-w-0">
            <p class="text-xs font-semibold text-gray-800 truncate">¿Conoces a <?= e($provider['full_name']) ?>?</p>
            <p class="text-[11px] text-gray-500">Recomiéndalo y ayúdalo a crecer</p>
        </div>
        <a href="https://wa.me/?text=<?= urlencode('Te recomiendo a ' . $provider['full_name'] . ($provider['tagline'] ? ' — ' . $provider['tagline'] : '') . '. Mira su perfil aquí: ' . $profileUrl) ?>"
           target="_blank" rel="noopener"
           class="flex-shrink-0 flex items-center gap-1.5 bg-[#25D366] text-white text-sm font-semibold px-4 py-2.5 rounded-xl">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
            Compartir
        </a>
        <button onclick="nativeShare()"
                class="flex-shrink-0 p-2.5 border border-gray-200 rounded-xl text-gray-500 hover:bg-gray-50">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
        </button>
    </div>
</div>
<script>
// Hide floating bar when user scrolls to the sidebar share card
const shareCard = document.querySelector('.bg-white.rounded-2xl.p-5.shadow-sm.border.border-gray-100:has(h3)');
const floatBar = document.getElementById('float-share');
if (floatBar && shareCard) {
    const obs = new IntersectionObserver(([e]) => {
        floatBar.style.transform = e.isIntersecting ? 'translateY(100%)' : 'translateY(0)';
    }, { threshold: 0.5 });
    obs.observe(shareCard);
